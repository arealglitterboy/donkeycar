...
# 0 = text is right-side up, 1 = rotated 90 degrees clockwise, 2 = 180 degrees (flipped), 3 = 270 degrees
...
#
# (deprecated) Pin numbering according to Broadcom numbers
STEERING_PWM_PIN = 13
...
SERVO_HBRIDGE_2PIN = {
    ...
    # provides servo pulse to steering servo
    "PWM_STEERING_PIN": "RPI_GPIO.BOARD.33",
    # used to compensate for PWM frequency differents from 60hz; NOT for adjusting steering range
    "PWM_STEERING_SCALE": 1.0,
    "PWM_STEERING_INVERTED": False,   # True if hardware requires an inverted PWM pulse
    # pwm value for full left steering (use `donkey calibrate` to measure value for your car)
...
# Serial device to use for communication. Can check with ls /dev/tty*
VESC_SERIAL_PORT = "/dev/ttyACM0"
VESC_HAS_SENSOR = True  # Whether or not the bldc motor is using a hall effect sensor
# Whether or not to automatically start the heartbeat thread that will keep commands alive.
VESC_START_HEARTBEAT = True
# baudrate for the serial communication. Shouldn't need to change this.
VESC_BAUDRATE = 115200
VESC_TIMEOUT = 0.05  # timeout for the serial communication
# VESC accepts steering inputs from 0 to 1. Joystick is usually -1 to 1. This changes it to -0.5 to 0.5
VESC_STEERING_SCALE = 0.5
# VESC accepts steering inputs from 0 to 1. Coupled with above change we move Joystick to 0 to 1
...
DC_STEER_THROTTLE = {
    ...
    # pwm pin produces duty cycle for forward drive
    "FWD_DUTY_PIN": "RPI_GPIO.BOARD.15",
    # pwm pin produces duty cycle for reverse drive
...
# How much travel with a single tick, in mm. Roll you car a meter and divide total ticks measured by 1,000
MM_PER_TICK = 12.7625
# if using GPIO, which GPIO board mode pin to use as input
ODOM_PIN = 13
...
# TFMINI
HAVE_TFMINI = False
# tfmini serial port, can be wired up or use usb/serial adapter
...
# (ps3|ps4|xbox|pigpio_rc|nimbus|wiiu|F710|rc3|MM1|custom) custom will run the my_joystick.py controller written by the `donkey createjs` command
...
# if AD0 pin is pulled high them address is 0x69, otherwise it is 0x68
IMU_ADDRESS = 0x68
# Digital Lowpass Filter setting (0:250Hz, 1:184Hz, 2:92Hz, 3:41Hz, 4:20Hz, 5:10Hz, 6:5Hz)
IMU_DLP_CONFIG = 0
...
# Max throttle to go fowrward. The bigger the faster
...
GYM_CONF = {"body_style": "donkey", "body_rgb": (
    128, 128, 128), "car_name": "Niko", "font_size": 100}
GYM_CONF["racer_name"] = "Ben"
GYM_CONF["country"] = "Ireland"
GYM_CONF["bio"] = "I race robots."

# when racing on virtual-race-league use host "trainmydonkey.com"
SIM_HOST = "127.0.0.1"
# this is the millisecond latency in controls. Can use useful in emulating the delay when useing a remote server. values of 100 to 400 probably reasonable.
...
# serial number of camera or None if you only have one camera (it will autodetect)
...
# How many times should the car reverse when detected a stop sign, set to 0 to disable reversing
STOP_SIGN_MAX_REVERSE_COUNT = 10
# Throttle during reversing when detected a stop sign
STOP_SIGN_REVERSE_THROTTLE = -0.5

# FPS counter
SHOW_FPS = False
# the interval in seconds for printing the frequency info into the shell
